 ...
 *
 * variant="inline"  — faixa horizontal larga, para inserir entre receitas na listagem
 * variant="sidebar" — card vertical compacto, para sidebar da receita individual
 */
export function LivroPromoBanner({ variant = "inline" }: LivroPromoBannerProps) {
  if (variant === "sidebar") {
    return (
      <Link
        href={LIVRO_URL}
        className="group block rounded-2xl overflow-hidden border border-amber-800/40 bg-gradient-to-b from-zinc-900 to-amber-950/30 hover:border-amber-600/60 transition-all duration-300 hover:shadow-lg hover:shadow-amber-900/20"
      >
        {/* Cover image */}
        ...
        <div className="p-4">
          ...
          <div className="flex items-center gap-1 mb-3">
            ...
            <span className="text-stone-500 text-[10px] ml-1">+1.200 compradores</span>
          </div>
  ...
  return (
    <Link
      href={LIVRO_URL}
      className="group flex flex-col sm:flex-row items-center gap-6 rounded-2xl border border-amber-800/40 bg-gradient-to-r from-zinc-900 via-amber-950/20 to-zinc-900 hover:border-amber-600/60 transition-all duration-300 hover:shadow-xl hover:shadow-amber-900/20 p-6 sm:p-8"
    >
      {/* Cover */}
      <div className="relative flex-shrink-0 w-28 sm:w-32 aspect-[3/4] rounded-xl overflow-hidden shadow-2xl shadow-black/50">
        <Image
          src="/images/capa-livro-de-receitas.png"
          alt="Manual da Cozinha Ancestral"
          fill
      ...
      <div className="flex-1 text-center sm:text-left">
        ...
        <div className="flex flex-wrap items-center justify-center sm:justify-start gap-4">
          <div className="flex items-center gap-1">
            ...
            <span className="text-stone-500 text-xs ml-1">+1.200 compradores</span>
          </div>
          <div className="flex items-center gap-2 text-amber-400 font-bold text-lg">
            <span className="text-stone-500 text-sm line-through font-normal">R$ 97</span>
            R$ 49,00
          </div>
        </div>
      </div>
